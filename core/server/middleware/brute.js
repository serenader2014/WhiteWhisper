...
export default function (options) {
    ...
    let failedCount;
    let remainingTime;
    let obj = _.extend({}, defaultOptions, options);
    let reset         = (clear) => {
    ...
    let handleBlock = (target) => {
        ...
        if (target.remainingTime - timeGap > 0) {
            ...
            return brute.update(ip, {ip:ip, count: count, lastVisit: now, remainingTime: remainingTime}).then(() => {
                return true;
            });
        } else {
    ...
    let handleNormal = (target) => {
        ...
        if (now.getTime() - firstVisit.getTime() < obj.limitTime) {
            if (count < obj.limitCount) {
                return brute.update(ip, {ip: ip, count: count, lastVisit: now, remainingTime: 0}).then(() => {
                    return false;
                });
            } else {
    ...
    let BruteMiddleware = (req, res, next) => {
        ip            = req.ip;
        now           = new Date();
        count         = 0;
        remainingTime = 0;
